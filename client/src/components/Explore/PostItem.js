...
const postItem = ({ post, toggleLike, auth }) => {
  ...
  return (
    ...
    <Link to={`/user/${post.user._id}`}><div className='item-user pl-2 pb-1'><small className='text-muted'>{ post.user.firstName } { post.user.lastName.slice(0, 1) + '.' }</small></div></Link>
